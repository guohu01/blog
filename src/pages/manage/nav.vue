  ...
  <aside class="demoo">
    <el-row class="tac" type="flex">
      <el-col :span="6" class="menu" :class="[this.isCollapse?'el-col-6-hide':'el-col-6']">
        <!-- menu标题 -->
        <p class="tab-p" style="margin: 0;">
          <img ref="img" :class="[this.isCollapse?'rocket-hide':'rocket']" :src="rocket" />
          <span :class="[this.isCollapse?'test-hide':'test']">MyTest</span>
        </p>
        <!-- 侧边导航栏 -->
        <el-menu id="elmenu" class="el-menu-vertical-demo mymenu" @open="handleOpen" @close="handleClose"
          background-color="#191A23" :default-active="active" text-color="#B0BBB4" active-text-color="#FFFFFF"
          :collapse="isCollapse" :class="[this.isCollapse?'el-menu-vertical-demo1':'el-menu-vertical-demo2']">
          <div v-for="item in Tabs" :key="item.name">
            <el-submenu v-if="item.childs && item.childs.length" :key="item.name" :index="item.name">
              <template slot="title">
                <i :class="item.class"></i>
                <span>{{item.title}}</span>
              </template>

              <el-menu-item @click="goTo(icon)" v-for="icon in item.childs" :key="icon.name" :index="icon.name">
      ...
      <el-col :span="18" :class="[this.isCollapse?'el-col-18-hide':'el-col-18']">
        ...
        <div :class="[this.isCollapse?'header-hide':'header']">
          <div>
            <button @click="change" class="handup">
              <span :class="[this.isCollapse?'icon iconfont icon-zhedie2':'icon iconfont icon-zhedie1']"></span>
            </button>
        ...
        <div :class="[this.isCollapse?'navtop-hide':'navtop']">
          <!-- 标签页 -->
          <el-tabs class="tabs" v-model="editableTabsValue" type="card" @tab-remove="removeTab" @tab-click="changetab">
            <el-tab-pane v-for="(item) in editableTabs" :key="item.name" :label="item.title" :name="item.name"
              :closable="item.close"></el-tab-pane>
          </el-tabs>
          <!-- 下拉选择框 -->
          <el-dropdown>
        ...
        Tabs: [{
            ...
            childs: [{
              ...
              },
              {
                ...
                close: true,
                class: "iconfont icon-pinglunguanli"
              },
              {
                title: "分类/标签",
                name: "2-3",
